...
Choose the category by the plan's primary purpose, intended use, expected output, and type of uncertainty. Do not classify work by its source or origin alone: extending a basic-research result does not automatically make the next plan applied research, and implementing a paper does not automatically make the plan experimental development. "Innovation" is not a primary R&D category label here; do not mix the contribution at publication time with later adoption, diffusion, or social value when classifying the plan.
...
The plan is the unit that must make a single category declaration. If secondary category work is incidental, record it as context only, not as another declared category inside the same plan. If it becomes load-bearing for a claim, decision, or report, use `ADJACENT` or a separate plan.
