...
5. Write the Plan section, starting with `### Plan visual` so the architecture, data flow, evaluation flow, mechanism, system boundary, variable space, decision flow, or derivation structure is inspectable before execution.
...
Reports must not include next-action or next-hypothesis sections; keep the report as an evidence artifact.
...
- **Plan visual before prose-only execution.** Every Plan section starts with `### Plan visual`. Use Mermaid, PlantUML, ASCII, or a durable linked figure/table to show the architecture, data flow, evaluation flow, mechanism, system boundary, variable space, decision flow, or derivation dependency. `No diagram:` is allowed only with a specific reason why no visual structure would help.
